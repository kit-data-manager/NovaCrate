...
test("Edit different Property Types (no links), then save", async ({ page }) => {
    ...
    await expect(page.locator("div:nth-child(4) > .grid > .bg-info")).toBeVisible()
    await expect(page.locator("div:nth-child(5) > .grid > .bg-info")).toBeVisible()
    await expect(page.getByText("There are unsaved changes")).toBeVisible()
    await page.getByRole("switch").first().click()
    await expect(page.locator("div:nth-child(7) > .grid > .bg-info")).toBeVisible()
    await page.getByRole("switch").first().click()
    await expect(page.locator("div:nth-child(7) > .grid > .bg-info")).not.toBeVisible()
    ...
    await expect(page.locator("div:nth-child(8) > .grid > .bg-info")).toBeVisible()
