...
The Profile system in NovaCrate is intended for use with multiple different profile mechanisms (called profile handlers in NovaCrate). As there currently is no official standard for machine
actionable profiles, NovaCrate aims to (enable) support (for) any of the existing profile mechanisms.

## Supported Profile Handlers
...
NovaCrate is open to contributions to support other profile handlers (e.g. Describo profiles, Crate-O modes, ...)

# Adding new Profile Handler

A NovaCrate **Profile Handler** is basically a specification that defines how to turn a [profile crate](https://www.researchobject.org/ro-crate/specification/1.3/profiles.html#profile-crate) that conforms to a specification or file format into a [NovaCrate profile definition](../lib/core/profiles/types/ProfileDefinition.ts).

To extend NovaCrate to support a new profile handler, the following steps have to be taken:

1. Implement the [ProfileHandler interface](../lib/core/profiles/IProfileHandler.ts). The profile interface currently has few requirements. Your implementation only needs to provide a NovaCrate profile definition based on the profile crate that is being parsed.
