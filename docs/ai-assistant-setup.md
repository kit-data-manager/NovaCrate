...
![Figure showing where to find the AI Assistant Toggle](./figures/ai-assistant-setup/find-ai-assistant.png)

# Setting up the AI Assistant within NovaCrate

![Figure showing how to open the AI Assistant settings and how to add a new provider](./figures/ai-assistant-setup/initial-settings.png)
...
4. Click on the "Add Provider" button. You should now see the following popup:
    ...
    ![Figure showing the "Configure AI Assistant" modal, explained in steps 5 to 10](./figures/ai-assistant-setup/pre-model-fetch.png)

5. Use a provider of your choice. You need an active (paid) subscription with OpenAI or Anthropic to use them. OpenRouter provides a small range of free models, but a registration is required to get an API key. You can also configure your own OpenAI-compatible provider if you have access to one.
...
10. The models available to the AI Assistant are configured here. It is recommended to fetch these models automatically using the provided button. For this to work, you need to complete the steps above (select a provider and provide an API Key).
   ...
   ![Figure showing the "Configure AI Assistant" modal, explained in steps 11 to 12](./figures/ai-assistant-setup/post-model-fetch.png)
