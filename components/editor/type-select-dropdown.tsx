    ...
    return (
        <Content>
            ...
            {props.propertyCanBe.canBeNumber ? (
                <DropdownMenuItem onClick={() => props.onPropertyTypeSelect(PropertyType.Number)}>
                    <Diff className="size-4 mr-2" /> Number
                </DropdownMenuItem>
            ) : null}
            {props.propertyCanBe.canBeBoolean ? (
                <DropdownMenuItem onClick={() => props.onPropertyTypeSelect(PropertyType.Boolean)}>
                    <Binary className="size-4 mr-2" /> Boolean
                </DropdownMenuItem>
            ) : null}
            {props.propertyCanBe.canBeTime ? (
                <DropdownMenuItem onClick={() => props.onPropertyTypeSelect(PropertyType.Time)}>
                    <Clock9 className="size-4 mr-2" /> Time
                </DropdownMenuItem>
            ) : null}
            {props.propertyCanBe.canBeDate ? (
                <DropdownMenuItem onClick={() => props.onPropertyTypeSelect(PropertyType.Date)}>
                    <Calendar className="size-4 mr-2" /> Date
                </DropdownMenuItem>
            ) : null}
            {props.propertyCanBe.canBeDateTime ? (
                <DropdownMenuItem onClick={() => props.onPropertyTypeSelect(PropertyType.DateTime)}>
